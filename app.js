...
function renderLists(){
  ...
  $('#circleList').innerHTML=state.circles.map((p,i)=>`<div class="item-card ${selected?.type==='circle'&&selected.id===p.id?'selected':''}" data-kind="circle" data-id="${p.id}"><i class="item-color circle-icon" style="border-color:${p.color}"></i><span>${p.name}<small>Radius ${p.radius}</small></span><button data-action="delete" title="Delete">×</button></div>`).join('');
  document.querySelectorAll('.item-card').forEach(el=>{el.onclick=(e)=>{const type=el.dataset.kind,id=+el.dataset.id,action=e.target.dataset.action;if(action==='delete'){remove(type,id);return}if(action==='up'||action==='down'){reorder(type,id,action==='up'?-1:1);return}selected={type,id};renderLists();hideInspector()};el.ondblclick=e=>{if(!e.target.dataset.action)renderInspector()}});
  function entityList(type,items,shape){return items.map((p,index)=>`<div class="item-card ${selected?.type===type&&selected.id===p.id?'selected':''}" data-kind="${type}" data-id="${p.id}"><i class="item-color ${shape}-item" style="background:${p.color}">${entityGlyph(type,index)}</i><span>${p.name}<small>${type[0].toUpperCase()+type.slice(1)} · ${index+1}</small></span><span class="reorder-actions"><button data-action="up" title="Move up" ${index===0?'disabled':''}>↑</button><button data-action="down" title="Move down" ${index===items.length-1?'disabled':''}>↓</button></span><button data-action="delete" title="Delete">×</button></div>`).join('')}
}
...
function reorder(type,id,direction){const keys={character:'characters',chapter:'chapters',scene:'scenes'},items=state[keys[type]];if(!items)return;const from=items.findIndex(item=>item.id===id),to=from+direction;if(from<0||to<0||to>=items.length)return;[items[from],items[to]]=[items[to],items[from]];save();renderLists();draw()}

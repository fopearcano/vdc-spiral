...
function hitTest(point){const layers=[['scene','scenes','scenesVisible'],['chapter','chapters','chaptersVisible'],['character','characters','charactersVisible']];for(const [type,key,visible] of layers){if(!state[visible])continue;for(const item of [...state[key]].reverse()){const q=screen(item);if(Math.hypot(point.x-q.x,point.y-q.y)<item.size+8)return{type,item}}}const c=center(),distance=Math.hypot(point.x-c.x,point.y-c.y),item=state.circlesVisible&&[...state.circles].reverse().find(circle=>Math.abs(distance-circle.radius*state.zoom)<7);return item?{type:'circle',item}:null}
...
canvas.addEventListener('wheel',e=>{e.preventDefault();state.zoom=Math.max(.05,Math.min(20,state.zoom*(e.deltaY>0?.86:1.16)));syncZoom();draw()},{passive:false});

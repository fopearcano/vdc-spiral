...
function drawEntities(){
  [['character','characters','charactersVisible'],['chapter','chapters','chaptersVisible'],['scene','scenes','scenesVisible']].forEach(([type,key,visible])=>{if(!state[visible])return;const scale=state[key+'Scale']/100,paint=state[key+'Paint'];state[key].forEach(p=>{const q=screen(p),size=p.size*scale;ctx.save();ctx.shadowColor='#17202322';ctx.shadowBlur=5;ctx.shadowOffsetY=2;ctx.fillStyle=p.color;ctx.strokeStyle=p.color;ctx.lineWidth=Math.max(1.5,size*.12);ctx.beginPath();if(type==='character')ctx.arc(q.x,q.y,size,0,Math.PI*2);if(type==='chapter')ctx.rect(q.x-size,q.y-size,size*2,size*2);if(type==='scene'){ctx.moveTo(q.x,q.y-size);ctx.lineTo(q.x+size,q.y+size);ctx.lineTo(q.x-size,q.y+size);ctx.closePath()}paint==='stroke'?ctx.stroke():ctx.fill();ctx.shadowColor='transparent';ctx.fillStyle=paint==='stroke'?p.color:'#fff';ctx.font=`600 ${Math.max(5,Math.min(p.textSize,size*1.2))}px DM Sans`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(p.initials,q.x,q.y+(type==='scene'?size*.2:0));ctx.fillStyle=state.theme==='dark'?'#e7ecea':'#28312f';ctx.font=`600 ${p.textSize}px DM Sans`;ctx.fillText(p.name,q.x,q.y+size+p.textSize+2);ctx.restore()})});
}
...
[['characters','charactersScale','charactersPaint'],['chapters','chaptersScale','chaptersPaint'],['scenes','scenesScale','scenesPaint']].forEach(([prefix,scaleKey,paintKey])=>{bindNumericPair(prefix+'Scale',scaleKey,1,500);const select=$('#'+prefix+'Paint');select.value=state[paintKey];select.onchange=()=>{state[paintKey]=select.value;save();draw()}});

...
const defaults = {
  ...
  goldenScale: 100, goldenWidth: 2, goldenColor: '#d79b2c', constructionOpacity: .25,
  constructionColor: '#8aa1a1',
  characters: [
...
function drawGrid(w,h,c){
  ctx.save(); ctx.strokeStyle='#e8ece9'; ctx.lineWidth=1;
...
function drawAxes(w,h,c){
  ctx.save();ctx.strokeStyle='#aeb7b4';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(0,c.y);ctx.lineTo(w,c.y);ctx.moveTo(c.x,0);ctx.lineTo(c.x,h);ctx.stroke();
  ctx.fillStyle='#87918f';ctx.font='9px DM Sans';ctx.textAlign='center';const step=56*state.zoom;
...
function drawCircles(c){state.circles.forEach(o=>{ctx.save();ctx.strokeStyle=o.color;ctx.globalAlpha=o.opacity;ctx.lineWidth=o.width;ctx.setLineDash([4,4]);ctx.beginPath();ctx.arc(c.x,c.y,o.radius*state.zoom,0,Math.PI*2);ctx.stroke();ctx.restore()})}
function drawConstruction(c){
  if(!state.construction||!state.fib)return;const n=state.sides,R=190*state.zoom;ctx.save();ctx.translate(c.x,c.y);ctx.strokeStyle=state.constructionColor;ctx.globalAlpha=state.constructionOpacity;ctx.lineWidth=1;
...
function drawSpirals(c){ctx.save();ctx.translate(c.x,c.y);if(state.golden){ctx.strokeStyle=state.goldenColor;ctx.globalAlpha=.9;ctx.lineWidth=state.goldenWidth;spiralPath(0,18*state.goldenScale/100,.22)}if(state.fib){ctx.strokeStyle='#d79b2c';ctx.globalAlpha=.82;ctx.lineWidth=1.65;for(let i=0;i<state.sides;i++)spiralPath(i*Math.PI*2/state.sides,11,.24)}ctx.restore()}
function drawCharacters(){state.characters.forEach(p=>{const q=screen(p);ctx.save();ctx.shadowColor='#17202322';ctx.shadowBlur=5;ctx.shadowOffsetY=2;ctx.fillStyle=p.color;ctx.beginPath();ctx.arc(q.x,q.y,p.size,0,Math.PI*2);ctx.fill();ctx.shadowColor='transparent';ctx.fillStyle='#fff';ctx.font=`600 ${Math.max(8,p.size*.55)}px DM Sans`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(p.initials,q.x,q.y);ctx.fillStyle='#28312f';ctx.font='600 10px DM Sans';ctx.fillText(p.name,q.x,q.y+p.size+12);ctx.restore()})}
...
function renderInspector(){
  if(!selected){$('#inspector').innerHTML='<div class="inspector-empty"><div>✦</div><h2>Select an element</h2><p>Choose a character or intimacy circle to edit its appearance.</p></div>';return}
  ...
  const isChar=selected.type==='character';$('#inspector').innerHTML=`<h2>${p.name}</h2><div class="type">${isChar?'CHARACTER':'INTIMACY CIRCLE'}</div>
    ...
    ${isChar?'':`<div class="field"><label>Stroke width <output>${p.width}</output></label><input id="editWidth" type="range" min="0.5" max="6" step="0.5" value="${p.width}"></div><div class="field"><label>Opacity <output>${Math.round(p.opacity*100)}%</output></label><input id="editOpacity" type="range" min="5" max="100" value="${p.opacity*100}"></div>`}
...
canvas.addEventListener('pointerdown',e=>{const r=canvas.getBoundingClientRect();for(const p of [...state.characters].reverse()){const q=screen(p);if(Math.hypot(e.clientX-r.left-q.x,e.clientY-r.top-q.y)<p.size+8){dragging=p;canvas.classList.add('dragging');canvas.setPointerCapture(e.pointerId);return}}});
...
[['goldenScale','goldenScale',Number],['goldenWidth','goldenWidth',Number],['goldenColor','goldenColor',String],['constructionColor','constructionColor',String]].forEach(([id,k,cast])=>{const el=$('#'+id);el.value=state[k];el.oninput=()=>{state[k]=cast(el.value);save();draw()}});
...
document.querySelectorAll('[data-collapse]').forEach(b=>b.onclick=()=>{const body=$('#'+b.dataset.collapse);body.hidden=!body.hidden;b.querySelector('b').textContent=body.hidden?'⌄':'⌃'});
function init(){renderLists();renderInspector();syncZoom();resize()} window.addEventListener('resize',resize);init();

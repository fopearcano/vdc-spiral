...
canvas.addEventListener('pointerdown',e=>{const r=canvas.getBoundingClientRect(),point={x:e.clientX-r.left,y:e.clientY-r.top};const layers=[['scene','scenes','scenesVisible'],['chapter','chapters','chaptersVisible'],['character','characters','charactersVisible']];for(const [type,key,visible] of layers){if(!state[visible])continue;for(const p of [...state[key]].reverse()){const q=screen(p);if(Math.hypot(point.x-q.x,point.y-q.y)<p.size+8){dragging=p;selected={type,id:p.id};renderLists();renderInspector();canvas.classList.add('dragging');canvas.setPointerCapture(e.pointerId);return}}}const c=center(),distance=Math.hypot(point.x-c.x,point.y-c.y),circle=state.circlesVisible&&[...state.circles].reverse().find(p=>Math.abs(distance-p.radius*state.zoom)<7);if(circle){selected={type:'circle',id:circle.id};renderLists();renderInspector();return}selected=null;renderLists();renderInspector()});
